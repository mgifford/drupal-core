...
final class Helper {
  ...
  public static function isActive(): bool {
    if (self::$active === NULL) {
      ...
      if ($admin_theme_name === 'default_admin') {
        return TRUE;
      }
      ...
      self::$active = array_key_exists('default_admin', $base_themes);
    }
  ...
  public static function isContentForm(?FormStateInterface $form_state = NULL, string $form_id = ''): bool {
    ...
    if ($form_id) {
      ...
      $form_ids_to_ignore = array_merge(\Drupal::moduleHandler()->invokeAll('default_admin_content_form_ignore_form_ids'), $form_ids_to_ignore);
    ...
    if (!isset($is_content_form_route)) {
      ...
      // API check.
      $additional_routes = \Drupal::moduleHandler()->invokeAll('default_admin_content_form_routes');
      $route_names = array_merge($additional_routes, $route_names);
      \Drupal::moduleHandler()->alter('default_admin_content_form_routes', $route_names);
      \Drupal::service('theme.manager')->alter('default_admin_content_form_routes', $route_names);
