...
| Evidence review path | Proposed `openacr-pilot/` package in `mgifford/drupal-core`, submitted from the Zivtech-owned fork | Current workflow sponsor corrected the repository preference | Pending upstream review; neither repository is the canonical Drupal core repository |
...
| Charter submitter | Zivtech, through its GitHub organization fork | Submit the Phase 0 proposal for upstream review | Represent upstream review as acceptance or issue an official Drupal core ACR |
| Charter drafter | OpenAI Codex, working for the current workflow sponsor | Draft and revise Phase 0 documents | Approve human authority or make product claims |
...
Unassigned collector and evaluator roles are deliberate stop states, but they
belong to different lanes. Phase 1 collection cannot begin until the proposed
pilot release-scope owner accepts the charter and a collector, repository
maintainer, and evidence reviewer are assigned in a reviewed amendment. An
evaluator remains unassigned until a separate evaluation engagement is approved;
evaluation staffing is not a prerequisite for the collection-only Phase 1.
...
- Someone describes the Zivtech fork, this pull request, or its upstream merge as
  Drupal Association or Drupal Core Leadership Team approval.
