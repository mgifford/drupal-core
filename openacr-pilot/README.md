...
> **Status:** Draft Phase 0 charter for human review. No collector, evaluation,
> Accessibility Conformance Report (ACR), or publication workflow exists here.
>
...
This package holds the proposed charter for a small Drupal 11 issue-traceability
pilot. Zivtech is submitting it to Mike Gifford's accessibility test-harness fork
for review. Neither the submission nor its repository location makes it an
official Drupal project artifact. The pilot tests whether a frozen set of public
Drupal.org issues can support remediation traceability without becoming evidence
of conformance.
...
The proposed release-scope owner and an independent scope reviewer must review the
charter. Approval would authorize planning for a read-only collector. It would not
authorize collection, evaluation, an ACR draft, issue filing, or publication. An
official Drupal core ACR would additionally require the Drupal Association to
accept the issuer role and name a representative, plus technical scope approval
from the Drupal Core Leadership Team.
